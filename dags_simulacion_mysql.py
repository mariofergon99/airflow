...
def mySQL_writer(cf):
    ...
    last_id_pd = pd.read_sql("SELECT ID FROM monitoring_system_events ORDER BY ID DESC LIMIT 1", con=mysql_db_conn)
    print(len(last_id_pd.index))
    print(len(last_id_pd))
    if last_id_pd.empty:
        print('entró')
        print(i)
        next_id = 0
    else:
        next_id = int(last_id_pd['ID'][0]) + 1
        print(i)
    print('------------------')
    print(next_id)
    print('------------------')

    for i in range(next_id , next_id + cf['airflow']['max_rows']):
